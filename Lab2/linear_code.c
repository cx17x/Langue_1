...
static void emit_node_ops(CodeBlock *b, const CFGNode *node) {
  for (int i=0;i<node->ops.n_lines;i++) {
    emit_comment(b, node->ops.lines[i]);
    emit_instruction(b, "  NOP");
  }
}
...
static void emit_branch(CodeBlock *b, const CFG *cfg, const char *const *labels, int node_id, const IntList *order) {
...
void program_image_add_function_from_cfg(ProgramImage *img, const ProgramFunction *func) {
  ...
  for (int i=0;i<order.n;i++) {
    ...
    if (node_id == entry) emit_prolog(block);
    if (!is_exit_role(node)) {
      emit_node_ops(block, node);
      emit_branch(block, cfg, (const char *const *)labels, node_id, &order);
    }
  }
